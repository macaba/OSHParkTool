...
namespace OSHParkTool
{
    class Program
    {
        ...
        private static void CreateMainFile()
        {
            ...
            {
                int i = 0;
                using (ZipArchive zip = ZipFile.Open(Path.Combine(directory, "Gerbers.zip"), ZipArchiveMode.Create))
                {
                    foreach (string file in files)
                    {
                        if (rules.Keys.Any(r => file.Contains(r)))
        ...
        private static void CreateStencilFile()
        {
            Dictionary<string, string> rules = new Dictionary<string, string>();
            ...
            //New proteus
            rules.Add("CADCAM Top SMT Paste.GBR", ".GBR");
            rules.Add("CADCAM Bottom SMT Paste.GBR", ".GBR");
            rules.Add("CADCAM Mechanical 1.GBR", ".GKO");     //Board outline
            rules.Add("CADCAM Mechanical 2.GBR", ".GKO");     //Board outline
            rules.Add("CADCAM Mechanical 3.GBR", ".GKO");     //Board outline
            rules.Add("CADCAM Mechanical 4.GBR", ".GKO");     //Board outline
            ...
            {
                int i = 0;
                using (ZipArchive zip = ZipFile.Open(Path.Combine(directory, "Stencil.zip"), ZipArchiveMode.Create))
                {
                    foreach (string file in files)
                    {
                        if (rules.Keys.Any(r => file.Contains(r)))
